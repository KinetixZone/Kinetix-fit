...
const WorkoutsView = ({ user }: { user: User }) => {
    ...
    // Estados para el Mago de Asignación (Assignment Wizard)
    const [assigningTemplate, setAssigningTemplate] = useState<Plan | null>(null);
    const [targetClient, setTargetClient] = useState<string>('');
    const [assignStep, setAssignStep] = useState<'select-athlete' | 'options' | 'customizing' | 'scheduling'>('select-athlete');
    const [finalPlanToSchedule, setFinalPlanToSchedule] = useState<Plan | null>(null);
    ...
    const startAssignment = (template: Plan) => {
        setAssigningTemplate(template);
        setTargetClient('');
        setAssignStep('select-athlete');
        setFinalPlanToSchedule(null);
    };

    const handleAthleteSelected = () => {
        if (!targetClient || !assigningTemplate) return;
        // Preparamos un clon base del plan para este usuario
        const basePlan: Plan = {
            ...assigningTemplate,
            id: generateUUID(), // Nuevo ID único para la asignación
            userId: targetClient,
            updatedAt: new Date().toISOString()
        };
        setFinalPlanToSchedule(basePlan);
        setAssignStep('options');
    };

    const handleCustomize = () => {
        setAssignStep('customizing');
    };

    const handleQuickAssign = () => {
        // En asignación rápida, guardamos el plan tal cual y pasamos a agendar
        if (finalPlanToSchedule) {
            DataEngine.savePlan(finalPlanToSchedule);
            setAssignStep('scheduling');
        }
    };

    const handleSaveCustomizedPlan = (updatedPlan: Plan) => {
        // Guardamos el plan personalizado y pasamos a agendar
        DataEngine.savePlan(updatedPlan);
        setFinalPlanToSchedule(updatedPlan); // Actualizamos ref por si acaso
        setAssignStep('scheduling');
    };
    ...
    return (
        <div className="space-y-6 animate-fade-in">
             ...
             {activeTab === 'routines' && (user.role === 'coach' || user.role === 'admin') && (
                 <RoutinesView onAssign={startAssignment} />
             )}
            ...
            {assigningTemplate && (
                <div className="fixed inset-0 bg-black/80 z-[80] flex items-center justify-center p-4 backdrop-blur-sm" onClick={closeAssignment}>
                    
                    {assignStep === 'select-athlete' && (
                        <div className="bg-[#1A1A1D] w-full max-w-md rounded-2xl p-6 border border-white/10 shadow-2xl" onClick={e => e.stopPropagation()}>
                            <h3 className="text-lg font-bold text-white mb-4">Asignar: {assigningTemplate.title}</h3>
                            <label className="text-[10px] text-gray-500 uppercase font-bold mb-2 block">1. Seleccionar Atleta</label>
                            <select 
                            ...
                            <div className="flex gap-3">
                                <button onClick={closeAssignment} className="flex-1 py-3 bg-white/5 rounded-xl font-bold text-sm text-gray-400 hover:text-white">Cancelar</button>
                                <button 
                                    onClick={handleAthleteSelected}
                                    disabled={!targetClient}
                                    className={`flex-1 py-3 rounded-xl font-bold text-sm text-white transition-all ${targetClient ? 'bg-blue-600 hover:bg-blue-500' : 'bg-gray-800 text-gray-500'}`}
                                >
                                    Siguiente
                                </button>
                            </div>
                    ...
                    {assignStep === 'options' && (
                        <div className="bg-[#1A1A1D] w-full max-w-md rounded-2xl p-6 border border-white/10 shadow-2xl text-center" onClick={e => e.stopPropagation()}>
                            <h3 className="text-xl font-bold text-white mb-2">Modo de Asignación</h3>
                            <p className="text-sm text-gray-400 mb-6">¿Deseas personalizar la rutina para este atleta antes de agendar?</p>
                            
                            <div className="space-y-3">
                                <button onClick={handleCustomize} className="w-full py-4 bg-white/5 border border-white/10 hover:border-blue-500 rounded-2xl flex items-center justify-between px-6 group transition-all">
                                    <div className="text-left">
                                        <div className="text-blue-400 font-bold text-sm group-hover:text-blue-300">Personalizar Rutina</div>
                                        <div className="text-[10px] text-gray-500">Editar ejercicios, cargas y notas para este usuario.</div>
                                    </div>
                                    <Edit3 size={20} className="text-gray-600 group-hover:text-blue-400"/>
                                </button>

                                <button onClick={handleQuickAssign} className="w-full py-4 bg-white/5 border border-white/10 hover:border-green-500 rounded-2xl flex items-center justify-between px-6 group transition-all">
                                    <div className="text-left">
                                        <div className="text-green-500 font-bold text-sm group-hover:text-green-400">Asignar & Agendar</div>
                                        <div className="text-[10px] text-gray-500">Usar la plantilla original sin cambios.</div>
                                    </div>
                                    <CalendarDays size={20} className="text-gray-600 group-hover:text-green-500"/>
                                </button>
                            </div>
                            <button onClick={() => setAssignStep('select-athlete')} className="mt-6 text-xs text-gray-500 hover:text-white underline">Volver</button>
                        </div>
                    )}

                    {assignStep === 'customizing' && finalPlanToSchedule && (
                        // Renderizamos el builder pero controlando que no se cierre todo, sino que avance
                        <div className="fixed inset-0 z-[90] bg-[#0A0A0C]" onClick={e => e.stopPropagation()}>
                             <ManualPlanBuilder 
                                plan={finalPlanToSchedule}
                                onSave={handleSaveCustomizedPlan} 
                                onCancel={() => setAssignStep('options')} 
                             />
                        </div>
                    )}

                    {assignStep === 'scheduling' && finalPlanToSchedule && (
                        <div className="absolute inset-0 z-10 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={e => e.stopPropagation()}>
                             <AssignRoutineModal 
                                ...
                                coach={user}
                                template={finalPlanToSchedule} // Usamos el plan final (ya guardado) para agendar
                                onClose={closeAssignment}
                                onSuccess={() => {
                                    // La persistencia del plan YA SE HIZO en los pasos previos (handleQuickAssign o handleSaveCustomizedPlan).
                                    // Aquí solo cerramos.
                                    closeAssignment();
                                }}
                             />
